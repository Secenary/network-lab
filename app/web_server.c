...
void http_respond(tcp_conn_t *tcp_conn, char *url_path, uint16_t port, uint8_t *dst_ip, uint16_t dst_port) {
    ...
    if (!file) {
        ...
        tcp_send(tcp_conn, (uint8_t *)resp_buffer, (uint16_t)strlen(resp_buffer), port, dst_ip, dst_port);
        ...
        // TODO: 发送 HTTP 响应体
        tcp_send(tcp_conn, (uint8_t *)not_found_body, (uint16_t)strlen(not_found_body), port, dst_ip, dst_port);
    ...
    while ((bytes_read = fread(resp_buffer, 1, sizeof(resp_buffer), file)) > 0) {
        // TODO: 每次发送读取的文件内容块
        tcp_send(tcp_conn, (uint8_t *)resp_buffer, (uint16_t)bytes_read, port, dst_ip, dst_port);
    }
