...
void tcp_out(tcp_conn_t *tcp_conn, buf_t *buf, uint16_t src_port, uint8_t *dst_ip, uint16_t dst_port, uint8_t flags) {
    /* =============================== TODO 1 BEGIN =============================== */
    // Step 1: 添加 TCP 报头
    if (buf_add_header(buf, sizeof(tcp_hdr_t)) < 0) {
        printf("tcp_out: buf_add_header failed\n");
        return;  // 添加失败，丢弃该包
    }
    ...
    // Step 2: 填充 TCP 首部字段
    tcp_hdr->src_port16 = swap16(src_port);        // 源端口号（注意字节序转换）
    tcp_hdr->dst_port16 = swap16(dst_port);        // 目标端口号
    tcp_hdr->seq = swap32(tcp_conn->seq);          // 序列号
    tcp_hdr->ack = swap32(tcp_conn->ack);          // 确认号
    tcp_hdr->doff = (sizeof(tcp_hdr_t) / 4) << 4;  // 首部长度（单位为 4 字节）
    tcp_hdr->flags = flags;                        // TCP 标志位
    tcp_hdr->win = swap16(TCP_MAX_WINDOW_SIZE);    // 窗口大小
    tcp_hdr->uptr = 0;                             // 紧急指针为 0

    // Step 3: 计算并填充校验和
    tcp_hdr->checksum16 = 0;  // 先清零
